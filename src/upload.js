  ...
  for (const file of fileList) {
    ...
    reader.onload = e => {
      try {
        ...
        window.State.uploadedFiles.push({ name: file.name, className, headers: validHeaders, data: records, idCol: headerRow[idCol], nameCol })
        renderFileList()
...
export function removeFile(idx) {
  window.State.uploadedFiles.splice(idx, 1)
  renderFileList()
}
